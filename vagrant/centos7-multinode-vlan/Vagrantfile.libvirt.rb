...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "master" do |node|
    ...
    # enabling the master ip on the Flat Network, for debug purposes only:
  ...
  config.vm.define "node01" do |node|
    ...
    # Openstack Network for Admin/Public/Mgmt
    node.vm.network :private_network, ip: "192.168.34.31"

    # Openstack Flat Network
    # no dhcp needed here from vagrant, dhcp via Openstack
    # the ip info here is a dummy config so Vagrant doesn't complain.
    node.vm.network :private_network, ip: "192.168.37.31",
      libvirt__network_name: "salt-os-flat",
      libvirt__netmask: "255.255.255.0",
      libvirt__dhcp_enabled: false,
      auto_config: false
  ...
  config.vm.define "node02" do |node|
    ...
    # Openstack Network for Admin/Public/Mgmt
    node.vm.network :private_network, ip: "192.168.34.32"

    # Openstack Flat Network
    # no dhcp needed here from vagrant, dhcp via Openstack
    # the ip info here is a dummy config so Vagrant doesn't complain.
    node.vm.network :private_network, ip: "192.168.37.32",
      libvirt__network_name: "salt-os-flat",
      libvirt__netmask: "255.255.255.0",
      libvirt__dhcp_enabled: false,
      auto_config: false
  ...
  config.vm.define "node03" do |node|
    ...
    # Openstack Network for Admin/Public/Mgmt
    node.vm.network :private_network, ip: "192.168.34.33"

    # Openstack Flat Network
    # no dhcp needed here from vagrant, dhcp via Openstack
    # the ip info here is a dummy config so Vagrant doesn't complain.
    node.vm.network :private_network, ip: "192.168.37.33",
      libvirt__network_name: "salt-os-flat",
      libvirt__netmask: "255.255.255.0",
      libvirt__dhcp_enabled: false,
      auto_config: false
